...
def parser_system_prompt() -> str:
    ...
    return (
        ...
        "The user may ask in English or Spanish. Map Spanish words to canonical internal values: "
        "estandar or olimpica to Standard; femenino or mujer to F; open, masculino, or hombre to O; "
        "natacion to Swim; ciclismo or bici to Bike; carrera or atletismo to Run; "
        "grupo por edad means age_group, while categoria may refer to sex plus age group such as F40-44. "
        "Always return canonical field values in English. If Context JSON includes response_language, "
        "write clarification questions in that language. "
        "If Context JSON includes previous.last_payload or previous.last_result, use it only for short "
        "follow-up questions in the same session. For example, 'what about 43:00?' may reuse the previous "
        "segment, modality, sex category, and age group. Do not reuse hidden context when it conflicts with "
        "the current user text. If the follow-up says Swim/natacion improved but the previous query only has "
        "a total time and no segment splits, ask for the missing split information or confirmation that the "
        "user wants to subtract the improvement from total time. "
