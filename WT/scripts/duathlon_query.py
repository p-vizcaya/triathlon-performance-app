...
PAIR_BY_SEGMENTS = {
    frozenset(("Run1", "Bike")): "run1_bike",
    frozenset(("Run1", "Run2")): "run1_run2",
    frozenset(("Bike", "Run2")): "bike_run2",
}
PAIR_SEGMENTS = {
    "run1_bike": ("Run1", "Bike"),
    "run1_run2": ("Run1", "Run2"),
...
def _cube_joint(percentiles: dict[str, float], modality: str, sex: str, age_group: str) -> float:
    return float(cube_query(modality, sex, age_group, percentiles["Run1"], percentiles["Bike"], percentiles["Run2"])["joint_rbr_percentile"])
...
def conditional_segment_query(
    ...
    if target_segment not in ("Run1", "Bike", "Run2"):
        raise ValueError("Conditional analysis supports Run1, Bike, and Run2 only.")
