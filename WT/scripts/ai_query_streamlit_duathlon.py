...
def _segment(locale: str, value: str) -> str:
    if locale == "es":
        return {"Run1": "Carrera 1", "Bike": "Bicicleta", "Run2": "Carrera 2", "Total": "Total"}.get(value, value)
    return value


def _modality(locale: str, value: str) -> str:
    return "Estándar" if locale == "es" and value == "Standard" else value
...
def _default_times() -> dict[str, str]:
    return {"Run1": "40:00", "Bike": "1:12:00", "Run2": "22:00"}
...
def _condition_controls(locale: str, index: int, available: list[str], defaults: dict[str, str]) -> dict:
    ...
    segment = st.selectbox(
        ...
        key=f"du_cond_{index}_segment",
    )
    operator = st.selectbox(
        _text(locale, f"Condition {index} operator", f"Operador de condición {index}"),
        CONDITION_OPERATORS,
        format_func=lambda value: _operator(locale, value),
        key=f"du_cond_{index}_operator",
    )
    result = {"segment": segment, "operator": operator}
    if operator == "between":
        left, right = st.columns(2)
        with left:
            result["lower_time"] = _time(_text(locale, "Lower time", "Tiempo inferior"), f"du_cond_{index}_lower", defaults[segment])
        with right:
            result["upper_time"] = _time(_text(locale, "Upper time", "Tiempo superior"), f"du_cond_{index}_upper", defaults[segment])
    else:
        result["time"] = _time(_text(locale, "Condition time", "Tiempo de la condición"), f"du_cond_{index}_time", defaults[segment])
    return result
...
def _guided_query(locale: str, modality: str, sex: str, age_group: str) -> None:
    ...
    if query == "Total time to percentile":
        value = _time(_text(locale, "Total time", "Tiempo total"), "du_total_time", "2:18:00")
        ...
        value = _time(_text(locale, "Segment time", "Tiempo del segmento"), "du_segment_time")
        if st.button(_text(locale, "Get answer", "Obtener respuesta"), type="primary"):
            _show_curve(locale, context, curve_query(modality, internal_sex, age_group, segment, value))
    elif query == "Percentile to total time":
        value = _percentile(_text(locale, "Percentile", "Percentil"), "du_total_pct")
        if st.button(_text(locale, "Get answer", "Obtener respuesta"), type="primary"):
    ...
    elif query in ("Estimated total from Run1/Bike/Run2", "Full split evaluation", "Compare segments"):
        run1 = _time(_segment(locale, "Run1"), f"du_profile_run1_{query}")
        bike = _time(_segment(locale, "Bike"), f"du_profile_bike_{query}", "1:12:00")
        run2 = _time(_segment(locale, "Run2"), f"du_profile_run2_{query}", "22:00")
        t1 = t2 = None
        if query != "Estimated total from Run1/Bike/Run2":
            t1 = _time("T1", f"du_profile_t1_{query}", "1:30")
            t2 = _time("T2", f"du_profile_t2_{query}", "1:30")
        if st.button(_text(locale, "Get answer", "Obtener respuesta"), type="primary"):
            _show_profile(locale, context, evaluate_profile(modality, internal_sex, age_group, run1, bike, run2, t1_time=t1, t2_time=t2))
        ...
        current = _time(_text(locale, "Current time", "Tiempo actual"), "du_gap_current")
        target = _percentile(_text(locale, "Target percentile", "Percentil objetivo"), "du_gap_target")
        if st.button(_text(locale, "Get answer", "Obtener respuesta"), type="primary"):
        ...
        target = _percentile(_text(locale, "Target total percentile", "Percentil total objetivo"), "du_missing_target")
        known = {segment: _time(_segment(locale, segment), f"du_missing_{segment}") for segment in MAIN_SEGMENTS if segment != missing}
        if st.button(_text(locale, "Get answer", "Obtener respuesta"), type="primary"):
        ...
        target_segment = st.selectbox(_text(locale, "Target segment", "Segmento objetivo"), MAIN_SEGMENTS, index=2, format_func=lambda value: _segment(locale, value), key="du_cond_target_segment")
        target_time = _time(_text(locale, "Target time", "Tiempo objetivo"), "du_cond_target_time", defaults[target_segment])
        first = _condition_controls(locale, 1, [segment for segment in MAIN_SEGMENTS if segment != target_segment], defaults)
        conditions = [first]
        if st.checkbox(_text(locale, "Add second condition", "Agregar segunda condición"), key="du_cond_add_second"):
        ...
        value = _percentile(_text(locale, "Percentile", "Percentil"), "du_explain_pct")
...
def render_duathlon_page(locale: str = "es") -> None:
    index = load_index()
    st.subheader(_text(locale, "Duathlon", "Duatlón"))
    st.caption(_text(locale, "Run1 - T1 - Bike - T2 - Run2", "Carrera 1 - T1 - Bicicleta - T2 - Carrera 2"))
    modality = st.sidebar.selectbox(_text(locale, "Modality", "Modalidad"), ("Standard", "Sprint"), format_func=lambda value: _modality(locale, value), key="du_modality")
    sex = st.sidebar.selectbox(_text(locale, "Sex", "Sexo"), ("M", "F"), format_func=lambda value: _text(locale, "Male" if value == "M" else "Female", "Masculino" if value == "M" else "Femenino"), key="du_sex")
    internal_sex = _internal_sex(sex)
    ...
    st.info(_text(locale, "Guided Duathlon queries, including conditional segment analysis, are enabled.", "Las consultas guiadas de duatlón, incluido el análisis condicional de segmentos, están habilitadas."))
